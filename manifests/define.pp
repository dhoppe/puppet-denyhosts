...
define denyhosts::define (
  ...
  $config_file_options_hash = $denyhosts::config_file_options_hash,
) {
  ...
  $_config_file_path  = pick($config_file_path, "${denyhosts::config_dir_path}/${name}")
  $_config_file_owner = pick($config_file_owner, $denyhosts::config_file_owner)
  $_config_file_group = pick($config_file_group, $denyhosts::config_file_group)
  $_config_file_mode = pick($config_file_mode, $denyhosts::config_file_mode)
  $config_file_content = default_content($config_file_string, $config_file_template)

  $_config_file_notify = pick($config_file_notify, $denyhosts::config_file_notify)
  $_config_file_require = pick($config_file_require, $denyhosts::config_file_require)
